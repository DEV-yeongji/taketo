	...
	<section class="localcourseList">
			...
			<div class="localBoard">
				<div class="localBoardListImg">
					...
					<div class="localBoardContents">
						...
						<br>
						<br>
						<span class="writeId">Write by. ${cList.nickName } </span>
					</div>
				</div>
			</div>
		</c:forEach>
		...
		<div class="writeBtn">
			<c:if test="${sessionScope.id ne null }">
				<a href="courseBoardWrite.do?id=${sessionScope.id }">글 작성하기 <i class="fas fa-pencil-alt"></i></a>
			</c:if>
		</div>
		<div class="pageing">
		<c:set var="curPage" value="${param.curPage }"/>
			...
			<c:forEach var="i" begin="0" end="${maxPage }">
				...
				<c:if test="${curPage ne i }">
					...
					<a href="localCourseSearch.do?curPage=${i }&city=${city}">${i+1 }</a>
				</c:if>
			</c:forEach>
		</div>
	</section>
