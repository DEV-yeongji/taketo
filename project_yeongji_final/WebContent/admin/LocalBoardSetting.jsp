...
<body>
	...
	<table class="themeBoardSet" border="1">
		...
		<tr>
			...
			<td>
				<a href="LocalBoardSetDeleteOK.admin?no=${list.no }">삭제</a> |
				<a href="localBoardView.do?no=${list.no }&&id=${list.id}">게시물확인</a>
			</td>
		</tr>
		</c:forEach>
	</table>
	<div class="paging">
				...
				<c:forEach var="i" begin="0" end="${maxPage }">
					...
					<c:if test="${curPage ne i }">
						<a href="LocalBoardSetting.admin?curPage=${i }">${i+1 }</a>
					</c:if>
				</c:forEach>
			</div>
</body>
