	...
	<section id="ThemeBoardList">
			<table class="ThemeBoardListTable">
				<c:forEach items="${list}" var="list" >
					<tr>
						<td><img src="./img/${list.fileName1 }"></td>
						<td class="listInfo">
							<p class="likeAndHit">
								<i class="fas fa-eye"></i> ${list.hit } 
							 </p>
							 <c:if test="${list.interesting eq '미식' }">
							 	<h2 class="themeInfo_food">
									<i class="fas fa-utensils"></i> ${list.interesting}
								</h2>
							 </c:if>
							 <c:if test="${list.interesting eq '힐링' }">
							 	<h2 class="themeInfo_healing">
									<i class="fas fa-heartbeat"></i> ${list.interesting}
								</h2>
							 </c:if>
							 <c:if test="${list.interesting eq '액티비티' }">
							 	<h2 class="themeInfo_act">
									<i class="fas fa-running"></i> ${list.interesting}
								</h2>
							 </c:if>
							 <c:if test="${list.interesting eq '자연' }">
							 	<h2 class="themeInfo_nature">
									<i class="fas fa-leaf"></i> ${list.interesting}
								</h2>
							 </c:if>
							<p class="listInfo"><a href="ThemeBoardView.do?no=${list.no}&&id=${list.id}"> " ${list.title } " </a></p>
							<span class="listInfo_name">${list.nickName } </span> 님의
							<span class="listInfo_location">${list.locationName }</span>
							<p class="listHashTag">${list.hashTag }</p>
						</td>
					</tr>
				</c:forEach>
			</table>
			...
			<div class="pageing">
				...
				<c:forEach var="i" begin="0" end="${maxPage }">
					...
					<c:if test="${curPage ne i }">
						<a href="ThemeBoard.do?curPage=${i }">${i+1 }</a>
					</c:if>
				</c:forEach>
			</div>
	</section>
